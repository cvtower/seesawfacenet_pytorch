...
class seesaw_Depth_Wise(Module):
     def __init__(self, in_c, out_c, residual = False, kernel=(3, 3), stride=(2, 2), padding=(1, 1), groups=1, use_se = 0, use_hs = 1):
     ...
     def forward(self, x):
        ...
        x1 = x[:, :(x.shape[1]*3//4), :, :]
        x2 = x[:, (x.shape[1]*3//4):, :, :]
        x1 = self.project_3(x1)
        x2 = self.project_1(x2)
        x = torch.cat((x1, x2), 1)
        if self.residual:
            output = short_cut + x
        else:
...
class seesaw_Residual(Module):
    def __init__(self, c, num_block, groups, kernel=(3, 3), stride=(1, 1), padding=(1, 1), use_se = 0, use_hs = 1):
...
##################################  seesaw_shareFaceNet #############################################################
class seesaw_shareFaceNet(Module):
    def __init__(self, embedding_size=512):
        super(seesaw_shareFaceNet, self).__init__()
        self.conv1 = Conv_block(3, 64, kernel=(3, 3), stride=(2, 2), padding=(1, 1))
        self.conv2_dw = Conv_block(64, 64, kernel=(3, 3), stride=(1, 1), padding=(1, 1), groups=64)
